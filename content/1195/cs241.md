...
The loader's core algorithm reads \(k\) words of machine code and copies them to `MEM[α + 4i]` for \(i = 1, \ldots, k\), sets the stack pointer \$30 to `α + 4n` (where \(n = k + \text{stack space}`), and returns \(\alpha\).
