...
<div class="remark">
<strong>BASE and eventual consistency as alternatives to ACID.</strong> Distributed NoSQL systems (Cassandra, DynamoDB, CouchDB) often relax ACID in favor of higher availability and partition tolerance. The alternative is described by the <strong>BASE</strong> acronym (Basically Available, Soft state, Eventually consistent), coined by Eric Brewer in contrast to ACID.
...
Cassandra achieves eventual consistency through <strong>anti-entropy</strong>: background processes on each node compare their data with replicas using Merkle tree digests, and any discrepancies are repaired by copying the newer version. Write conflicts are resolved by <strong>last-write-wins</strong> (using a timestamp embedded in each write) — a simple rule that is incorrect when clocks are skewed, but acceptable for many applications where the window of conflict is small.
...
<div class="remark">
<strong>Materialized views and query rewriting.</strong> An advanced optimizer feature is <strong>automatic view matching</strong>: even if a query does not explicitly reference a materialized view, the optimizer can detect that the query's algebraic expression is a subset of a materialized view's expression, and rewrite the query to use the view instead of the base tables.
...
<em>Write</em>: all writes go to an in-memory sorted structure (the <strong>MemTable</strong>, typically a skip list or red-black tree). When the MemTable reaches a size threshold (e.g., 64 MB), it is flushed to disk as an immutable <strong>SSTable</strong> (Sorted String Table) file at Level 0 (L0). The flush is a single sequential write — no random I/O.
