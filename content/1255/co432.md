...
\]
where \((X, Y) \sim \mu\) and \(\Pi = \Pi(X,Y)\) denotes the transcript. The <em>information complexity</em> of \(f\) is \(\mathrm{IC}_\mu(f) = \inf_\Pi \mathrm{IC}_\mu(\Pi)\) over all protocols computing \(f\).
</div>
