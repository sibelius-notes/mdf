...
| Design Type | Description | Strengths |
|---|---|---|
| **Block design** | Alternating blocks of task and control conditions | High statistical power; simple analysis |
| **Event-related design** | Brief, randomly ordered trials | Can isolate individual trial types; avoids habituation |
| **Resting-state fMRI** | No explicit task; measures spontaneous fluctuations | Reveals intrinsic functional connectivity networks |
...
<strong>Hindbrain (Rhombencephalon):</strong> Comprises the <strong>medulla oblongata</strong>, <strong>pons</strong>, and <strong>cerebellum</strong>. The medulla controls vital autonomic functions (respiration, cardiac rhythm, blood pressure). The pons relays information between cortex and cerebellum and contains nuclei important for sleep and arousal. The <strong>cerebellum</strong> coordinates motor timing, balance, and motor learning and increasingly is recognized as contributing to cognitive and emotional processing.
</div>

<div class="definition">
<strong>Midbrain (Mesencephalon):</strong> Contains the <strong>superior colliculi</strong> (visual orienting), <strong>inferior colliculi</strong> (auditory processing), <strong>substantia nigra</strong> (dopamine production critical for motor initiation; degeneration causes Parkinson's disease), and the <strong>periaqueductal grey</strong> (pain modulation and defensive behaviours).
</div>

<div class="definition">
<strong>Forebrain (Prosencephalon):</strong> Divided into the <strong>diencephalon</strong> (thalamus, hypothalamus) and the <strong>telencephalon</strong> (cerebral cortex, basal ganglia, limbic structures). The forebrain is the seat of higher cognitive functions.
</div>
...
| Tract Type | Description | Examples |
|---|---|---|
| **Association fibres** | Connect regions within the same hemisphere | Arcuate fasciculus, superior longitudinal fasciculus, inferior fronto-occipital fasciculus, uncinate fasciculus, cingulum |
| **Commissural fibres** | Connect homologous regions across hemispheres | Corpus callosum, anterior commissure |
| **Projection fibres** | Connect cortex with subcortical structures | Internal capsule, corona radiata |
...
<div class="remark">
<strong>Clinical Significance:</strong> The finding that even <em>partial</em> callosotomy patients showed the same deficit as complete callosotomy patients suggests that normal moral reasoning requires full interhemispheric integration. The right temporoparietal junction, which is critical for processing others' beliefs and intentions, must communicate with left hemisphere language areas and anterior right hemisphere regions involved in integrating social information. When this communication is disrupted, patients default to an outcome-based moral calculus that disregards mental states.
</div>
...
<div class="definition">
<strong>Ventral Stream ("Vision for Perception"):</strong> Projects from V1 through V2, V4, and into the <strong>inferotemporal cortex (IT)</strong>. This pathway constructs rich, detailed, conscious representations of objects---their identity, colour, texture, and semantic associations. It operates in an <strong>allocentric</strong> (object-centred) reference frame and is influenced by contextual information, prior knowledge, and visual illusions.
</div>

<div class="definition">
<strong>Dorsal Stream ("Vision for Action"):</strong> Projects from V1 through V2, V3, V5/MT, and into the <strong>posterior parietal cortex (PPC)</strong>. This pathway computes the spatial parameters needed for visually guided actions---reaching, grasping, eye movements. It operates in an <strong>egocentric</strong> (body-centred) reference frame, uses real-time visual information, and is relatively immune to visual illusions.
</div>
...
<div class="remark">
<strong>Research Highlight:</strong> Konkle and Caramazza (2013) discovered a <strong>tripartite organization</strong> of the ventral visual stream based on two fundamental dimensions: <strong>animacy</strong> (animate vs. inanimate) and <strong>real-world size</strong> (big vs. small). Using fMRI, they showed that cortical zones with distinct response preferences for big objects, all animals, and small objects are arranged in a spoked pattern around the occipital pole along a ventromedial-to-lateral-to-dorsomedial axis. Critically, the size distinction applied primarily within the <em>object</em> domain---big and small animals activated overlapping neural territories, whereas big and small objects (e.g., buildings vs. tools) activated substantially different regions. Furthermore, these preference zones were <em>duplicated</em> across both the ventral and lateral cortical surfaces, suggesting a major division of labour into two sub-streams within the ventral visual pathway. This work fundamentally changed understanding of how object categories are mapped onto cortical topography, showing that the organization reflects not arbitrary categorical boundaries but deep cognitive dimensions---animacy and physical interaction properties---that shape our engagement with the world.
</div>
...
<div class="definition">
<strong>Alerting Network:</strong> Maintains tonic and phasic readiness to respond. Depends on the <strong>locus coeruleus--norepinephrine</strong> system, right frontal cortex, and right parietal cortex. Phasic alerting occurs in response to warning signals; tonic alerting refers to baseline arousal levels.
</div>

<div class="definition">
<strong>Orienting Network:</strong> Selects information from sensory input by directing attention to particular spatial locations. Key structures include the <strong>superior parietal lobule</strong> (disengaging attention), <strong>temporoparietal junction</strong> (reorienting to unexpected stimuli), <strong>frontal eye fields</strong> (voluntary saccades), and <strong>superior colliculus</strong> (reflexive orienting). A distinction is drawn between <strong>endogenous</strong> (voluntary, goal-driven) orienting and <strong>exogenous</strong> (reflexive, stimulus-driven) orienting.
</div>

<div class="definition">
<strong>Executive Control Network:</strong> Resolves conflicts, monitors errors, and enables cognitive control. Centred on the <strong>anterior cingulate cortex (ACC)</strong> and the <strong>dorsolateral prefrontal cortex (DLPFC)</strong>. This network is engaged when automatic responses must be overridden in favour of goal-appropriate behaviour.
</div>

### Anterior Cingulate Cortex and Foraging Decisions

<div class="remark">
<strong>Research Highlight:</strong> Hayden, Pearson, and Platt (2011) provided a striking demonstration of the ACC's role in monitoring the value of current behaviour relative to alternatives. Recording from neurons in the dorsal ACC of macaques performing a foraging task in a patchy environment, they found that ACC neurons encoded a <em>decision variable</em> signalling the relative value of abandoning a depleting resource to seek a new one. As a patch's yield declined, ACC neuronal firing progressively increased and reached a fixed threshold that triggered patch-leaving behaviour. Furthermore, longer travel times to the next patch decreased the gain of "stay" responses and raised the threshold for leaving---consistent with the <strong>marginal value theorem</strong> from optimal foraging theory. This work established that the ACC does not merely detect conflicts or errors but actively computes the opportunity cost of persisting with the current course of action, a function central to adaptive attentional allocation in both ecological and laboratory settings.
</div>
...
<div class="remark">
<strong>Research Highlight:</strong> Saj, Fuhrman, Vuilleumier, and Boroditsky (2014, <em>Psychological Science</em>) demonstrated that left hemispatial neglect extends beyond spatial cognition to affect <strong>temporal representation</strong>. Exploiting the finding that many people represent time as a left-to-right mental timeline (with the past on the left and the future on the right), they showed that neglect patients had selective difficulty representing events associated with the <em>past</em>---that is, events falling on the "left side" of the mental timeline. This finding supports the hypothesis that spatial and temporal representations share common neural substrates and that intact spatial processing is necessary for at least some forms of temporal cognition. The metaphorical mapping of time onto space is not merely a linguistic convention but a deep cognitive structure that can be disrupted by the same brain damage that impairs spatial attention.
</div>
...
<div class="definition">
<strong>Sensory Memory:</strong> Ultra-brief storage (iconic memory for vision, ~250 ms; echoic memory for audition, ~2--4 s) that holds a large amount of information in a modality-specific format before it decays or is transferred to short-term storage.
</div>

<div class="definition">
<strong>Short-Term / Working Memory:</strong> A limited-capacity system that maintains and manipulates information over seconds to minutes. <strong>Baddeley's model</strong> proposes a <strong>central executive</strong> (attentional control), a <strong>phonological loop</strong> (verbal rehearsal), a <strong>visuospatial sketchpad</strong> (spatial and visual maintenance), and an <strong>episodic buffer</strong> (integrating information across subsystems and long-term memory). Working memory depends on the <strong>prefrontal cortex</strong> (particularly DLPFC for executive control), <strong>parietal cortex</strong> (spatial working memory), and <strong>Broca's area / premotor cortex</strong> (phonological rehearsal).
</div>

<div class="definition">
<strong>Long-Term Memory:</strong> Enduring storage of information, subdivided into:
<ul>
<li><strong>Explicit (declarative) memory</strong>: Conscious recollection of facts (<em>semantic memory</em>) and personal events (<em>episodic memory</em>). Depends on the <strong>medial temporal lobe</strong> (hippocampus, entorhinal, perirhinal, parahippocampal cortices).</li>
<li><strong>Implicit (nondeclarative) memory</strong>: Unconscious memory expressed through performance rather than conscious recall. Includes <strong>procedural memory</strong> (motor skills, habits; basal ganglia), <strong>priming</strong> (facilitated processing of previously encountered stimuli; sensory cortex), and <strong>classical conditioning</strong> (cerebellum for motor conditioning; amygdala for emotional conditioning).</li>
</ul>
</div>
...
<div class="remark">
<strong>Research Highlight:</strong> Oflaz et al. (in press, <em>Journal of Psychiatric Research</em>) investigated memory functioning in <strong>dissociative identity disorder (DID)</strong>, a condition in which individuals function as two or more distinct identity states. DID poses a unique challenge for memory science: patients often report amnesia for events experienced by alternate identities, suggesting that memory encoding and retrieval may be state-dependent or compartmentalized. The study provided evidence that amnesic barriers between identity states affect explicit memory (particularly autobiographical and episodic recall) but may be more porous for implicit memory, with some information transferring between identity states via implicit processes such as priming. This dissociation between explicit and implicit memory across identity states parallels the explicit/implicit dissociation observed in organic amnesia, suggesting that similar memory systems are involved but are modulated by dissociative processes rather than structural brain damage.
</div>
...
<div class="remark">
<strong>Research Highlight:</strong> Decety et al. (2014) provided a comprehensive neuroscientific framework for understanding <strong>empathy</strong>, proposing that it comprises three interacting components, each with distinct neural substrates:
<ol>
<li><strong>Affective sharing (emotional contagion):</strong> Mediated by the <em>anterior insula</em>, <em>anterior cingulate cortex</em>, and <em>somatosensory cortex</em>. These regions activate both when experiencing an emotion oneself and when observing the same emotion in another, consistent with a "shared representation" or simulation mechanism.</li>
<li><strong>Empathic understanding (cognitive empathy / perspective-taking):</strong> Mediated by the <em>medial prefrontal cortex</em>, <em>temporoparietal junction</em>, and <em>superior temporal sulcus</em>---regions associated with theory of mind and mentalizing. This component enables understanding <em>why</em> another person feels as they do.</li>
<li><strong>Empathic concern and emotion regulation:</strong> Mediated by the <em>ventromedial prefrontal cortex</em> and <em>dorsolateral prefrontal cortex</em>, which regulate empathic arousal to prevent personal distress and enable prosocial motivation.</li>
</ol>
The framework emphasizes that empathy is not a single process but a dynamic interplay between bottom-up affective resonance and top-down cognitive regulation. Clinical implications include understanding empathy deficits in psychopathy (impaired affective sharing but potentially intact cognitive empathy), autism (impaired cognitive empathy but potentially intact affective sharing), and alexithymia (impaired interoceptive awareness disrupting emotional self-knowledge and empathic response).
</div>
...
| Region | Function | Associated Deficit When Damaged |
|---|---|---|
| **Dorsolateral PFC** | Working memory, planning, abstract reasoning | Impaired strategy formation, poor WCST performance |
| **Ventrolateral PFC** | Controlled retrieval from memory, response selection | Impaired interference resolution, retrieval deficits |
| **Orbitofrontal cortex** | Reward/punishment evaluation, impulse control | Disinhibition, poor social judgment, personality change |
| **Anterior cingulate cortex** | Conflict monitoring, error detection, motivation | Apathy, reduced error awareness, impaired decision-making |
| **Frontopolar cortex (area 10)** | Multitasking, prospective memory, metacognition | Impaired branching, poor prospective memory |

### Prefrontal Cortex Structure and Executive Performance

<div class="remark">
<strong>Research Highlight:</strong> Hummer et al. (2014) conducted a meta-analysis examining the relationship between PFC neuroanatomy and executive cognition in healthy adults. Across 31 samples measuring PFC volume and 10 samples measuring PFC cortical thickness, comprising 3,272 participants in total, they found that both larger PFC volume and greater PFC cortical thickness were significantly associated with better executive function performance. The strength of this association varied depending on the specific executive task used, suggesting that different executive components may have different structural correlates within the PFC. Furthermore, greater variance in sample age (reflecting a wider age range) strengthened the PFC volume--executive function association, while mean age alone did not predict the strength of association. This finding is consistent with the idea that age-related variability in PFC structure drives some of the individual differences in executive function seen across the lifespan.
</div>
...
<div class="remark">
<strong>Research Highlight:</strong> Scheibel (2017) reviewed evidence that <strong>traumatic brain injury (TBI)</strong> disrupts normal DMN function. Patients with moderate-to-severe TBI showed a failure to deactivate the DMN during cognitive control tasks---the anticorrelation between the DMN and task-positive networks was weakened or absent. This failure to suppress internally directed processing during externally directed tasks was associated with impaired cognitive control performance. The underlying mechanism appears to involve white matter damage: reduced fractional anisotropy in fibres connecting the <strong>salience network</strong> (anterior insula, anterior cingulate) correlated with both reduced DMN deactivation and slower inhibitory performance. The salience network normally acts as a "switch" between the DMN and task-positive networks; when this switching mechanism is damaged, patients have difficulty transitioning between internally and externally focused states, contributing to the distractibility, poor sustained attention, and executive dysfunction commonly observed after TBI.
</div>
...
| Condition | DMN Abnormality |
|---|---|
| **Alzheimer's disease** | Reduced DMN connectivity; amyloid deposition preferentially targets DMN nodes |
| **Depression** | Hyperactive DMN; excessive rumination and self-focused thought |
| **Schizophrenia** | Failure to suppress DMN during tasks; blurred boundary between internal and external reality |
| **ADHD** | Intrusion of DMN activity during task performance; lapses of attention |
| **Autism** | Reduced DMN connectivity; impaired self-referential and social cognition |
...
| Cognitive Domain | Age Trajectory |
|---|---|
| **Processing speed** | Declines steadily from early adulthood; considered a fundamental factor driving other cognitive declines |
| **Working memory** | Declines, particularly for complex span tasks requiring executive control |
| **Episodic memory** | Declines; encoding and retrieval both affected, especially free recall |
| **Executive functions** | Decline, particularly inhibition, cognitive flexibility, and multitasking |
| **Semantic memory** | Relatively preserved or even increases into late adulthood |
| **Vocabulary / crystallized intelligence** | Preserved or increases until the 7th--8th decade |
...
<div class="remark">
<strong>Research Highlight:</strong> Nguyen et al. (in press, <em>Journal of Clinical and Experimental Neuropsychology</em>) investigated the moderating role of cognitive reserve in the relationship between age and cognitive performance. Their findings supported the hypothesis that higher cognitive reserve---operationalized through education, vocabulary, and reading ability---attenuates the negative impact of aging on both executive functions and verbal episodic memory. This work adds to a growing body of evidence that lifelong intellectual engagement provides a buffer against age-related cognitive decline, though it does not prevent the underlying neural deterioration. The clinical implication is that promoting cognitive engagement across the lifespan may delay the onset of functional impairment even as the brain accumulates age-related structural changes.
</div>
...
<div class="remark">
<strong>Research Highlight:</strong> Menon et al. (2011) proposed that schizophrenia can be understood as a disorder of <strong>large-scale brain network</strong> dysfunction, with the <strong>salience network</strong> playing a central role. The salience network, anchored in the <strong>anterior insula</strong> and <strong>anterior cingulate cortex</strong>, normally identifies the most relevant internal and external stimuli and coordinates appropriate responses by switching between the <strong>default mode network</strong> (internal mentation) and the <strong>central executive network</strong> (externally directed cognition). In schizophrenia, aberrant salience network function leads to inappropriate attribution of significance to irrelevant stimuli---potentially accounting for hallucinations (aberrant salience assigned to internal representations) and delusions (aberrant significance assigned to neutral events). Furthermore, failure of the salience network to properly suppress the DMN during task performance contributes to cognitive deficits, while failure to appropriately engage the central executive network underlies impaired goal-directed behaviour. This <strong>triple network model</strong> provides a unifying framework that connects the diverse symptoms of schizophrenia to dysfunction in a single switching mechanism.
</div>
...
<div class="definition">
<strong>Dorsal Stream (Articulation):</strong> Maps acoustic speech representations onto articulatory motor representations. Projects from the posterior superior temporal gyrus through the <strong>Sylvian fissure parietal-temporal boundary (area Spt)</strong> to premotor and prefrontal regions. Damage produces <strong>conduction aphasia</strong> and <strong>speech apraxia</strong>.
</div>

<div class="definition">
<strong>Ventral Stream (Comprehension):</strong> Maps acoustic speech representations onto lexical-semantic representations. Projects from the superior temporal gyrus anteriorly and ventrally through the <strong>middle and inferior temporal gyri</strong>. Damage produces <strong>word deafness</strong> and <strong>semantic deficits</strong>. This stream is more bilaterally organized than the dorsal stream.
</div>
...
| Aphasia Type | Fluency | Comprehension | Repetition | Lesion Site |
|---|---|---|---|---|
| **Broca's aphasia** | Non-fluent, effortful, agrammatic | Relatively preserved (though syntactic comprehension may be impaired) | Impaired | Left inferior frontal gyrus, adjacent premotor cortex |
| **Wernicke's aphasia** | Fluent, paraphasic, empty of content | Severely impaired | Impaired | Left posterior superior temporal gyrus |
| **Conduction aphasia** | Fluent with phonemic paraphasias | Relatively preserved | Severely impaired (disproportionate to other deficits) | Arcuate fasciculus, supramarginal gyrus, posterior insula |
| **Global aphasia** | Non-fluent | Severely impaired | Severely impaired | Large left perisylvian lesion |
| **Transcortical motor** | Non-fluent | Preserved | Preserved | Left frontal (anterior/superior to Broca's area) |
| **Transcortical sensory** | Fluent, paraphasic | Impaired | Preserved | Left temporo-parietal, posterior to Wernicke's area |
| **Anomic aphasia** | Fluent | Preserved | Preserved | Various; left temporal lobe, angular gyrus |

### Language Disorders After Subcortical Damage

<div class="remark">
<strong>Research Highlight:</strong> Zhang et al. (2013) investigated language disturbances resulting from subcortical lesions, contributing to the understanding that aphasia is not exclusively a cortical phenomenon. Damage to the <strong>left basal ganglia</strong> (particularly the caudate nucleus and putamen), <strong>thalamus</strong> (particularly the pulvinar and anterior nuclei), and <strong>subcortical white matter</strong> can produce language deficits that do not fit neatly into classical cortical aphasia categories. Subcortical aphasias often feature relatively preserved repetition, fluctuating symptoms, and a mix of fluent and non-fluent features. The mechanism may involve disruption of cortico-striato-thalamo-cortical loops that modulate cortical language processing, or disconnection of cortical language areas through white matter damage. These findings underscore that language depends on distributed networks rather than isolated cortical centres.
</div>
