...
| | Cooperate | Defect |
|---|---|---|
| <strong>Cooperate</strong> | 2, 2 | 5, 0 |
| <strong>Defect</strong> | 0, 5 | 4, 4 |
...
![Best response correspondences for Matching Pennies: $BR_1(q)$ (blue) and $BR_2(p)$ (red) intersect at the unique mixed Nash equilibrium $(p,q)=(1/2,1/2)$ (green star).](/pics/co759/best_response.png)
...
| | Stop | Go |
|---|---|---|
| <strong>Stop</strong> | 0, 0 | 0, 1 |
| <strong>Go</strong> | 1, 0 | −5, −5 |
...
![Myerson's Lemma: piecewise-constant monotone allocation rule $x_i(z)$ (left) and its uniquely determined payment $p_i(z)$ — the area to the left of the allocation curve (right).](/pics/co759/myerson_lemma.png)
...
![Vickrey auction with reserve price: expected revenue vs reserve price $r$ for 2 bidders Uniform$[0,1]$ (left); virtual valuation $\varphi(v)=2v-1$, excluding $v<1/2$ from allocation (right).](/pics/co759/vickrey_myerson.png)
...
\[
\sum_{e \in E} (f_e^<em> - f_e) c_e(f_e) \geq 0. \tag{</em>}
\]
...
\[
f_e^<em> \cdot c_e(f_e^</em>) \geq \frac{1}{\alpha(\mathcal{C})} \cdot f_e \cdot c_e(f_e) + (f_e^* - f_e) \cdot c_e(f_e).
\]
Summing over all edges:

\[
C(f^<em>) \geq \frac{1}{\alpha(\mathcal{C})} \cdot C(f) + \underbrace{\sum_e (f_e^</em> - f_e) c_e(f_e)}_{\geq 0 \text{ by } (*)}.
\]
...
\[
\text{cost}(\mathbf{P}) \leq \Phi(\mathbf{P}) \leq \Phi(\mathbf{P}^<em>) \leq \mathcal{H}_k \cdot \text{cost}(\mathbf{P}^</em>).
\]
...
\[
\sum_{i=1}^k C_i(s_i^<em>, \mathbf{s}_{-i}) \leq \lambda \cdot \text{cost}(\mathbf{s}^</em>) + \mu \cdot \text{cost}(\mathbf{s}).
\]
For payoff-maximization games, smoothness requires:

\[
\sum_{i=1}^k \pi_i(s_i^<em>, \mathbf{s}_{-i}) \geq \lambda \cdot V(\mathbf{s}^</em>) - \mu \cdot V(\mathbf{s}).
\]
</div>
...
<div class="proof">
<strong>Proof.</strong> Let \(\sigma\) be a CCE and \(\mathbf{s}^<em>\) an optimal outcome. Starting from the CCE condition — for each player \(i\), the unconditional expected cost of following \(\sigma\) is at most that of deviating to \(s_i^</em>\):
...
\]
The first inequality uses \(\text{cost}(\mathbf{s}) \leq \sum_i C_i(\mathbf{s})\) and the second uses the CCE condition for each \(i\). Now apply the smoothness condition (which holds for <em>all</em> \(\mathbf{s}\), hence under expectation):

\[
\leq \mathbf{E}_{\mathbf{s} \sim \sigma}[\lambda \cdot \text{cost}(\mathbf{s}^<em>) + \mu \cdot \text{cost}(\mathbf{s})] = \lambda \cdot \text{cost}(\mathbf{s}^</em>) + \mu \cdot \mathbf{E}[\text{cost}(\mathbf{s})].
\]
...
\]
After \(T\) rounds: \(W_{T+1} \leq n \cdot \prod_t (1 - \varepsilon \cdot \mathbf{E}[c_t])\). For the best action \(i^<em>\): \(W_{T+1} \geq w_{T+1}(i^</em>) = (1-\varepsilon)^{\sum_t c_t(i^*)}\). Taking logarithms and using \(\ln(1-x) \leq -x\) and \(\ln(1-\varepsilon) \geq -\varepsilon - \varepsilon^2\) gives the bound.
</div>
