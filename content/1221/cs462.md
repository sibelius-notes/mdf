...
<div class="definition">
<strong>Definition 2.2 (Mealy Machine)</strong>: A <strong>Mealy machine</strong> is a tuple \(M = (Q, \Sigma, \Delta, \delta, \lambda, q_0)\) where \(\lambda : Q \times \Sigma \to \Delta\) is the output function depending on the current state *and* the current input symbol.
</div>
...
\]
Two words are \(\equiv_L\)-equivalent if they are indistinguishable by any right extension. The equivalence classes of \(\equiv_L\) are the **Myhill-Nerode classes** of \(L\).
</div>
...
**Base case** \(|\Sigma| = 1\): Words are of the form \(a^n\). Any infinite sequence must have a non-decreasing pair by the infinite pigeonhole principle on lengths. ✓
...
<div class="theorem">
<strong>Theorem 3.4 (Parikh, 1966)</strong>: For any CFL \(L\), the set \(\Psi(L)\) is semilinear. Consequently, every CFL is **letter-equivalent** to some regular language: there exists a regular language \(R\) with \(\Psi(L) = \Psi(R)\).
</div>
...
<div class="proof">
<strong>Proof of Lemma</strong>: \(L(r_{M,w})\) is the complement of the language of valid accepting computation histories of \(M\) on \(w\). A string \(s\) is an *invalid* computation history if:
...
<div class="definition">
<strong>Definition 7.1 (DOL System)</strong>: A <strong>Deterministic OL system (DOL system)</strong> is a triple \((V, h, w)\) where \(V\) is an alphabet, \(h : V^* \to V^*\) is a morphism (the production rules, one per symbol), and \(w \in V^*\) is the **axiom** (starting word). The language generated is the sequence:
...
<div class="definition">
<strong>Definition 7.4 (Rational Series)</strong>: A formal power series \(S\) over \(\mathbb{K}\) is <strong>rational</strong> if it can be computed by a **weighted finite automaton** (WFA): an NFA where each transition is labeled with an element of \(\mathbb{K}\), and the coefficient \((S, w)\) is the sum (over \(\mathbb{K}\)) of products of edge weights along all accepting paths labeled \(w\).
</div>
...
<div class="proof">
<strong>Proof sketch</strong>: Given an NBA \(B\) with \(n\) states, the DMA construction proceeds by the **Safra construction** (1988). The states of the DMA are **Safra trees**: annotated trees whose nodes are labeled with subsets of the NBA's state space. A Safra tree represents a "summary" of all possible nondeterministic runs of \(B\) seen so far.

On reading each symbol, the Safra construction updates the tree deterministically:
1. **Powerset step**: each node's label is updated by applying the NBA transition function to all states in that label.
2. **Subset merging**: nodes track which copies of the NBA have visited accepting states, and "merge" them by promoting a subset to a new child node when it becomes stable.
3. **Tree normalization**: remove empty nodes and normalize the tree.
...
<div class="definition">
<strong>Definition 11.7 (Star-Free Language)</strong>: The <strong>star-free languages</strong> over \(\Sigma\) are the smallest collection containing all finite languages and closed under Boolean operations and concatenation (but *not* Kleene star).
</div>
