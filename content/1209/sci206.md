...
Direct current is what we have seen so far, current travels in one direction. For simple model, voltage difference is constant. Alternating current, direction of the current keeps changing. For example, \(V=k\cdot \sin(t) \). Different frequencies in different places.
