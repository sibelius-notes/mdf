...
![Level sets (ellipses) of $f(x_1,x_2)=x_1^2+2x_2^2$ with gradient vectors $\nabla f$ pointing outward, perpendicular to each contour.](/static/pics/co367/level_sets_gradient.png)
...
![Convex function $f(x)=x^2+1$ (left) with chord above curve and single global minimum; non-convex $f(x)=\sin(2x)+0.3x^2-0.5$ (right) with multiple local minima.](/static/pics/co367/convex_nonconvex.png)
...
![Gradient descent path on elliptic bowl $f(x_1,x_2)=x_1^2+3x_2^2$, showing iterates converging to the global minimum at the origin.](/static/pics/co367/gradient_descent.png)
...
![Second-order cone $\{(x,t):\|x\|_2\le t\}$: 3D cone surface (left) and cross-sectional disks at increasing values of $t$ (right).](/static/pics/co367/second_order_cone.png)
