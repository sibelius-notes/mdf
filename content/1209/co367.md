...
Given a function \( f : \mathbb{R}^n \to \mathbb{R} \), an **unconstrained optimization problem** takes the form \[ \min_{x \in \mathbb{R}^n} f(x). \] A **constrained optimization problem** (nonlinear program, NLP) is written as \[ \min f(x) \quad \text{subject to} \quad g_i(x) \leq 0,\ i \in I,\ h_j(x) = 0,\ j \in E, \] where \( f \) is the **objective function**, \( g_i \) are **inequality constraints**, and \( h_j \) are **equality constraints**. Nonlinear optimization (also called nonlinear programming) describes problems in which the objective or constraint functions are nonlinear in the variables.
...
**Proposition 1.5 (Cauchy-Schwarz Inequality).** For all \( x, y \in \mathbb{R}^n \), \[ |x^\top y| \leq \|x\|_2 \|y\|_2, \] with equality if and only if \( x = \alpha y \) for some scalar \( \alpha \).
...
A function \( f : E \to \mathbb{R}^m \) is **differentiable** at \( x_0 \) if there exists a linear map \( Df_{x_0} \) such that \[ \lim_{h \to 0} \frac{\|f(x_0+h) - f(x_0) - Df_{x_0}(h)\|}{\|h\|} = 0. \]
...
**Theorem 3.3 (Taylor's Theorem, univariate).** If \( f \) is \( k \)-times differentiable on an open interval containing \( a \) and \( x \), then for some \( c \) between \( a \) and \( x \), \[ f(x) = f(a) + f'(a)(x-a) + \frac{f''(a)}{2!}(x-a)^2 + \cdots + \frac{f^{(k-1)}(a)}{(k-1)!}(x-a)^{k-1} + \frac{f^{(k)}(c)}{k!}(x-a)^k. \]

**Theorem 3.6 (Taylor's Theorem, multivariate).** If \( f : \mathbb{R}^n \to \mathbb{R} \) is continuously differentiable, then for any \( x, d \in \mathbb{R}^n \), \[ f(x+d) = f(x) + \nabla f(x + td)^\top d \] for some \( t \in (0,1) \). If \( f \in C^2 \), then additionally \[ f(x+d) = f(x) + \nabla f(x)^\top d + \frac{1}{2} d^\top \nabla^2 f(x + td) d \] for some \( t \in (0,1) \).
...
**Theorem 4.3.** For symmetric \( A \) with smallest eigenvalue \( \lambda_{\min} \) and largest \( \lambda_{\max} \), \[ \lambda_{\min} \|x\|_2^2 \leq x^\top Ax \leq \lambda_{\max} \|x\|_2^2 \quad \forall x \in \mathbb{R}^n. \]
...
**Definition 5.8.** Let \( C \) be convex. A function \( f : C \to \mathbb{R} \) is **convex** if for all \( x, y \in C \) and \( \alpha \in (0,1) \), \[ f((1-\alpha)x + \alpha y) \leq (1-\alpha)f(x) + \alpha f(y). \] The function is **strictly convex** if the inequality is strict for \( x \neq y \). Examples: \( a^\top x + b \) (affine, convex); \( \max(0, x) \); \( x^2 \), \( e^x \) (strictly convex).
...
For the strongly convex quadratic \( f(x) = \frac{1}{2} x^\top Ax + b^\top x \) with \( A \succ 0 \) and eigenvalues \( \lambda_1 \geq \lambda_2 \geq \cdots \geq \lambda_n > 0 \), the exact-line-search steepest descent satisfies \[ f(x^{(k+1)}) - f(x^*) \leq \left(\frac{\lambda_1 - \lambda_n}{\lambda_1 + \lambda_n}\right)^2 (f(x^{(k)}) - f(x^*)). \] The **condition number** \( \kappa = \lambda_1/\lambda_n \) governs convergence speed: when \( \kappa \) is large (ill-conditioned), the factor approaches 1 and convergence is very slow. If \( A = \alpha I \), convergence happens in one step.
...
The approximation is constrained to satisfy the **secant equation**: \[ B_{k+1} s^{(k)} = y^{(k)}, \] where \( s^{(k)} = x^{(k+1)} - x^{(k)} \) and \( y^{(k)} = \nabla f(x^{(k+1)}) - \nabla f(x^{(k)}) \). This mirrors the property of the exact Hessian of a quadratic function, where \( A(x-y) = \nabla f(x) - \nabla f(y) \).

The **BFGS method** (Broyden-Fletcher-Goldfarb-Shanno) works with \( B^{(k)} = H_k^{-1} \) directly, so the search direction requires only a matrix-vector product (\( O(n^2) \) instead of \( O(n^3) \). Let \( \tau_k = 1/(y^{(k)\top} s^{(k)}) \); the update is \[ B^{(k+1)} = (I - \tau_k s^{(k)} (y^{(k)})^\top) B^{(k)} (I - \tau_k y^{(k)} (s^{(k)})^\top) + \tau_k s^{(k)} (s^{(k)})^\top. \]
...
Given \( A \in \mathbb{R}^{m \times n} \) with \( m > n \) and \( \text{rank}(A) = n \), the overdetermined system \( Ax = b \) is generally inconsistent. The **linear least squares problem** minimizes the residual: \[ \min_{x \in \mathbb{R}^n} \|Ax - b\|_2^2. \] Since \( \nabla^2 f(x) = 2A^\top A \succ 0 \) (as \( \text{rank}(A) = n \), the unique solution is \( x_{\text{LS}} = (A^\top A)^{-1} A^\top b \), obtained by solving the **normal equations** \( A^\top A x = A^\top b \).
...
A **constrained optimization problem** (NLP) is \[ \min f(x) \quad \text{s.t.} \quad g_i(x) \leq 0,\ i \in I;\ h_j(x) = 0,\ j \in E. \] A special case is linear programming (LP): \( \min c^\top x \) subject to \( Ax = b, x \geq 0 \). A **convex program (CP)** is an NLP in which \( f, g_1, \ldots, g_m \) are all convex and \( h_1, \ldots, h_p \) are affine.
...
**Definition 18.7 (Linearized feasible directions).** The set of **linearized feasible directions** is \[ L_F(\bar{x}) = \{d : \nabla g_i(\bar{x})^\top d \leq 0,\ \forall i \in A(\bar{x}) \cap I;\ \nabla h_j(\bar{x})^\top d = 0,\ \forall j \in E\}. \]
...
The **dual objective** function is \[ q(\lambda, \mu) := \min_{x \in D} L(x, \lambda, \mu), \] where \( \lambda \geq 0 \). The **dual problem** is \( \max q(\lambda, \mu) \) over its domain. The dual is always a convex program (even when the primal is not), as \( q \) is a pointwise infimum of affine functions.
...
The **Markowitz portfolio optimization** problem is \[ \min_w \, w^\top \Sigma w \quad \text{s.t.} \quad \bar{r}^\top w \geq r_{\min},\ e^\top w = 1, \] where \( w \in \mathbb{R}^n \) is the portfolio weight vector, \( \bar{r} \) is the vector of expected returns, \( \Sigma \) is the covariance matrix (assumed PSD) and \( r_{\min} \) is the minimum acceptable expected return. The objective minimizes **variance** (risk), subject to a return lower bound and budget constraint.

### Gradient Projection Method

For \( \min_{x \in \Omega} f(x) \) where \( \Omega \) is closed and convex, the **projection** \( P_\Omega(\bar{x}) \) is the unique closest point in \( \Omega \) to \( \bar{x} \). The **gradient projection method** iterates: \[ x^{(k+1)} = P_\Omega(x^{(k)} - \alpha_k \nabla f(x^{(k)})). \]
...
**Theorem 21.6 (Convergence of gradient projection).** If \( f \in C^1 \) is convex on \( \Omega \) with Lipschitz gradient (constant \( L \), and constant step size \( \alpha \in (0, 1/L] \) is used, then \[ f(x^{(m)}) - f(x^*) \leq \frac{\|x^* - x^{(0)}\|_2^2}{2\alpha m}. \] This is a \( O(1/m) \) convergence rate, which is slower than Newton's quadratic rate but applies to any convex problem.

## Lecture 22: Penalty Methods

### Quadratic Penalty Method

The **quadratic penalty method** approximates a constrained problem by adding a penalty for constraint violation. For equality constraints \( h_j(x) = 0 \), define the **penalized objective** \[ Q(x; \mu) = f(x) + \mu \sum_{j=1}^p h_j(x)^2, \] where \( \mu > 0 \) is the **penalty parameter**. As \( \mu \to +\infty \), constraint violation is penalized more heavily and the minimizers of \( Q \) approach the feasible region.
...
For general NLPs with both equalities and inequalities, the penalty function extends to \[ Q(x, \mu) = f(x) + \mu \sum_{j \in E} h_j(x)^2 + \mu \sum_{i \in I} \max(0, g_i(x))^2. \]
...
For inequality constraints \( g_i(x) \leq 0 \), the **log-barrier function** is \[ B(x, \rho) = f(x) - \rho \sum_{i \in I} \log(-g_i(x)), \quad \rho > 0, \] defined only on the **strictly feasible** interior \( \{x : g_i(x) < 0\} \). As \( \rho \to 0 \), the barrier approximation \( -\rho \log(-u) \) approaches the indicator function of \( \{u \leq 0\} \). Decreasing \( \rho \to 0 \) drives the barrier minimizers towards the boundary.
...
For the convex program \( \min f(x) \) s.t. \( g_i(x) \leq 0 \), \( Ax = b \) with a Slater point, define the **log-barrier** \[ B(x, \rho) = f(x) - \rho \sum_{i=1}^m \log(-g_i(x)). \] For each \( \rho > 0 \), the minimizer \( x_\rho \) of \( B \) over \( \{Ax = b\} \) is the **central point**. The set of central points \( \{x_\rho : \rho > 0\} \) is the **central path**.
...
For the LP pair \( (P): \max c^\top x, Ax = b, x \geq 0 \) and \( (D): \min b^\top y, A^\top y - s = c, s \geq 0 \), the central path conditions are \[ A^\top y + s - c = 0,\ Ax - b = 0,\ x_j s_j = \rho \quad \forall j, \] where \( x > 0, s > 0 \). **Primal-dual interior-point methods** apply Newton's method to this system for fixed \( \rho \), then decrease \( \rho \). Modern LP solvers (e.g., in MATLAB, Python) typically use this approach.

### Newton's Method for Equality Constraints

For \( \min f(x) \) s.t. \( Ax = b \) with \( f \in C^2 \) convex, Newton's method minimizes the second-order approximation: \[ \min_{d} \, f(x) + \nabla f(x)^\top d + \frac{1}{2}d^\top \nabla^2 f(x) d \quad \text{s.t.} \quad A(x+d) = b. \] The KKT conditions for this QP give the **Newton system** \[ \begin{pmatrix} \nabla^2 f(x) & A^\top \\ A & 0 \end{pmatrix} \begin{pmatrix} d \\ w \end{pmatrix} = \begin{pmatrix} -\nabla f(x) \\ 0 \end{pmatrix}, \] and the Newton update is \( x \leftarrow x + d \). Under suitable conditions, this converges superlinearly or quadratically, and combined with the log-barrier method, this forms the backbone of modern interior-point algorithms.
